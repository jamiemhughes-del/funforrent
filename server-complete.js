...
const server = createServer(async (req, res) => {
  ...
  if (pathname === "/api/inventory" && req.method === "GET") {
    ...
    try {
      ...
      // Return blocked time slots for this date
      const blockedSlots = (config.blockoutTimes || [])
        .filter(bt => bt.date === date)
        .map(bt => ({ start: bt.start, end: bt.end, reason: bt.reason || "" }));
    } catch { /* ignore */ }
  ...
  if (pathname === "/api/checkout" && req.method === "POST") {
    try {
      ...
      const squareRes = await squareApi("/v2/online-checkout/payment-links", {
        ...
        checkout_options: {
          redirect_url: `${url.protocol}//${url.host}/success.html?ref=${bookingId}`,
