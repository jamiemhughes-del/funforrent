...
async function sendEmail(to, subject, html, text) {
  if (!SENDGRID_API_KEY) {
    console.log("[EMAIL SKIPPED] No SENDGRID_API_KEY configured");
    return { skipped: true };
  }
  try {
    const res = await fetch("https://api.sendgrid.com/v3/mail/send", {
      ...
      body: JSON.stringify({
        ...
        from: { email: BUSINESS_EMAIL, name: "FunForRent" },
        subject,
        content: [
...
const server = createServer(async (req, res) => {
  ...
  if (pathname === "/api/booking-confirm" && req.method === "POST") {
    try {
      ...
      let emailStatus = { business: false, customer: false, skipped: false };
      if (booking) {
        ...
        if (bizResult.skipped) emailStatus.skipped = true;
        ...
        if (custResult.skipped) emailStatus.skipped = true;
      }
...
server.listen(PORT, () => {
  ...
  console.log(`Email: ${SENDGRID_API_KEY ? "enabled (SendGrid)" : "disabled (set SENDGRID_API_KEY)"}`);
});
